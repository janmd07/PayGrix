...
export function RiskParameters({ lendingData }: RiskParametersProps) {
  ...
  const referencePrice = lendingData?.collateralPrice || (isBase ? "2,500.00" : "60,000.00");
  const contractAddressShort = lendingData?.contractAddressShort || (isBase ? "0x7C5e...34b8" : "0x800C...22aE");
  const oracleAddressShort = lendingData?.oracleAddressShort || (isBase ? "0x204e...7067" : "0xA17B...2287");
  ...
  const collateralLabel = isBase ? "WETH (18 decimals)" : "cirBTC (8 decimals)";
  const oracleTypeLabel = isBase ? "Chainlink ETH/USD Adapter" : "Testnet Simulation Oracle";
    ...
    <Card className="border border-white/10 bg-[#060f24]/60 backdrop-blur-lg relative overflow-hidden shadow-[0_8px_32px_rgba(6,15,36,0.5)]">
      ...
      <CardHeader className="pb-3">
        ...
        <CardDescription className="text-xs text-slate-400">
          On-chain parameters read directly from {isBase ? "BaseSepoliaLending contract on Base Sepolia." : "PayGrixLending Phase 3C contract on Arc Testnet."}
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        <div className="rounded-xl border border-white/5 bg-[#070e1c] divide-y divide-white/5 text-xs">
          ...
          <div className="flex justify-between items-center p-3">
            ...
            <span className="text-purple-300 font-mono font-bold text-[11px]">
              {oracleAddressShort} ({oracleTypeLabel})
            </span>
          </div>
          ...
          <div className="flex justify-between items-center p-3">
            ...
            <span className="text-white font-mono font-bold flex items-center gap-1.5">
              <span className={`h-2 w-2 rounded-full ${isBase ? "bg-blue-400" : "bg-amber-400"}`} />
              {collateralLabel}
            </span>
          </div>
        ...
        <p className="text-[11px] text-slate-500 italic leading-relaxed pt-1 border-t border-white/5">
          {isBase 
            ? `* Base Sepolia environment. Chainlink ETH/USD reference price: $${referencePrice} / WETH.`
